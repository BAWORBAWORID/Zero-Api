...
const { processing } = require('../lib/funtions')
...
async function cekKey(req, res, next) {
    ...
    if (!apikey) return res.json({
        status: false, creator: `${creator}`, message: "[!] masukan parameter apikey"
    })
    ...
    if (db === null) {
        return res.json({
            status: false, creator: `${creator}`, message: "[!] Apikey Tidak Wujud"
        })
    } else if (!db.isVerified) {
        return res.json({
            status: false, creator: `${creator}`, message: "[!] Sila Verify Email dulu sebelum guna apikey"
        })
    } else if (db.limitApikey === 0) {
        return res.json({
            status: false, creator: `${creator}`, message: "[!] Apikey Sudah Habis"
        })
    } else {
        return next();
    }
...
async function premKey(req, res, next) {
    var apikey = req.query.apikey
    if (!apikey) return res.json({
        status: false, creator: `${creator}`, message: "[!] masukan parameter apikey"
    })
    ...
    if (db === null) {
        ...
        return res.json({
            status: false, creator: `${creator}`, message: "[!] Silahkan Verify Email dulu sebelum guna apikey"
        })
    } else if (!db.isPremium) {
        return res.json({
            status: false, creator: `${creator}`, message: "[!] Silahkan Upgrade Premium"
        })
    } else if (db.limitApikey === 0) {
        return res.json({
            status: false, creator: `${creator}`, message: "[!] Apikey Sudah Habis"
...
/*router.get("/images/:q(*)", (req, res) => {
    const file = __dirname + "/public/images/" + req.params.q;
...
router.get('/checkapikey', async (req, res, next) => {
    ...
    res.json({
        ...
        result: {
            ...
            Email: `${db.email}`,
            Verify: `${db.isVerified}`,
            Apikey: `${apikey}`,
            Limit: `${db.limitApikey}`,
            Premium: `${db.isVerified}`,
            Expired: `${db.expired}`
        }
    })
...
router.get('/api/dowloader/fbdown', cekKey, async (req, res, next) => {
    ...
    if (!url) return res.json({
        status: false, creator: `${creator}`, message: "[!] masukan parameter url"
    })
...
router.get('/api/dowloader/igstorydowloader', cekKey, async (req, res, next) => {
    ...
    if (!username) return res.json({
        status: false, creator: `${creator}`, message: "[!] masukan parameter username"
    })
...
router.get('/api/ai/enhance', premKey, async (req, res, next) => {
var url = req.query.url
...
router.get('/api/ai/gbard', premKey, async (req, res, next) => {
    var text = req.query.text;
    if (!text) return res.json({
        status: false, creator: `${creator}`, message: "[!] masukan parameter text"
    });

    const response = await axios.get(`https://api.akuari.my.id/ai/gbard?chat=${encodeURIComponent(text)}`);
...
router.get('/api/ai/dalle', premKey, async (req, res) => {
    try {
        ...
        const headers = {
            ...
            'Referer': 'https://art.aiseo.ai/',
            'authority': 'art.aiseo.ai',
            'Connection': 'keep-alive',
            'Cache-Control': 'max-age=0',
            'Upgrade-Insecure-Requests': '1'
        };

        const response = await axios.post('https://us-central1-code-translate-c9640.cloudfunctions.net/OpenAiImageGeneration', jsonData, { headers });

        //res.status(response.status).json({ result: response.data.result });
...
router.get('/api/ai/lexica', premKey, async (req, res, next) => {
  try {
    const cari = req.query.text;
        if (!cari) return res.json({
            status: false, creator: `${creator}`, message: "[!] masukan parameter text"
        });
    ...
    const postData = {
      text: cari, // Ganti self.query dengan nilai yang Anda inginkan
      searchMode: 'images',
      source: 'search',
      model: 'lexica-aperture-v2'
    };

    const response = await axios.post('https://lexica.art/api/infinite-prompts', postData);
    
    // Memproses respons untuk mengambil ids dari gambar
    const imageIds = response.data.prompts.map(prompt => prompt.images.map(image =>  `https://image.lexica.art/full_jpg/${image.id}`)).flat();
    //const imageUrls = prompts.flatMap(prompt => prompt.images.map(image => `https://image.lexica.art/full_jpg/${image.id}`));

    // Mengambil secara acak satu ID gambar
    const result = getRandomId(imageIds);

    // Mengirim kembali ID gambar secara acak ke klien
...
router.get('/api/ai/prodia', premKey, async (req, res, next) => {
     const text = req.query.text;
     if (!text) return res.json({
        status: false, creator: `${creator}`, message: "[!] masukan parameter text"
     });
    ...
    try {
        const response = await axios.post('https://api.prodia.com/generate', jsonData, { headers });
        
        const response2 = await axios.get(`https://api.prodia.com/job/${response.data.job}`);
        const result = `https://images.prodia.xyz/${response2.data.job}.png`;
...
router.get('/api/ai/gpt-4', premKey, async (req, res, next) => {
    try {
        ...
        const response = await axios.post('https://us-central1-aiseo-official.cloudfunctions.net/newApiFree', jsonData, { headers });
...
router.get('/api/ai/gpt-3', premKey, async (req, res, next) => {
    try {
...
router.get('/api/ai/gpt-3.5-turbo', premKey, async (req, res, next) => {
    try {
...
router.get('/api/textpro/glitch2', cekKey, async (req, res, next) => {
    ...
    if (!text2) return res.json({
        status: false, creator: `${creator}`, message: "[!] masukan parameter text2"
    })
...
router.get('/api/soundoftext', cekKey, async (req, res, next) => {
    ...
    if (!lan) return res.json({
        status: false, creator: `${creator}`, message: "[!] sila letak format lang dengan betul cek web site https://soundoftext.com/docs untuk lihat code lang"
    })

    textto.sounds.create({
        text: text1, voice: lan
    })
    .then(soundUrl => {
        limitapikey(req.query.apikey)
        res.json({
...
router.get('/api/maker/circle', cekKey, async (req, res) => {
    ...
    if (!img) return res.json({
        status: false, creator: 'Alip', message: "[!] cek kembali url image"
    })
...
router.get('/api/maker/darkness', cekKey, async (req, res) => {
    ...
    if (!no) return res.json({
        status: false, creator: `${creator}`, message: "[!] masukan parameter no"
    })
    ...
    if (!n) return res.json({
        status: false, creator: 'Alip', message: "[!] parameter no nombor sahaja"
    })
...
router.get('/api/maker/resize', cekKey, async (req, res) => {
    ...
    if (!width) return res.json({
        status: false, creator: `${creator}`, message: "[!] masukan parameter width"
    })
    if (!height) return res.json({
        status: false, creator: `${creator}`, message: "[!] masukan parameter height"
    })
    ...
    if (!wid) return res.json({
        status: false, creator: 'Alip', message: "[!] parameter width nombor sahaja"
    })
    if (!hei) return res.json({
        status: false, creator: 'Alip', message: "[!] parameter height nombor sahaja"
    })
...
router.get('/api/maker/emojimix', cekKey, async (req, res, next) => {
    ...
    if (!emoji1) return res.json({
        status: false, creator: `${creator}`, message: "[!] masukan parameter emoji1"
    })
    if (!emoji2) return res.json({
        status: false, creator: `${creator}`, message: "[!] masukan parameter emoji2"
    })
...
router.get('/api/maker/welcome1', cekKey, async (req, res, next) => {
    ...
    if (!name) return res.json({
        status: false, creator: 'Alip', message: "[!] masukan parameter name"
    })
    if (!grup) return res.json({
        status: false, creator: 'Alip', message: "[!] masukan parameter gpname"
    })
    if (!member) return res.json({
        status: false, creator: 'Alip', message: "[!] masukan parameter member"
    })
    if (!pp) return res.json({
        status: false, creator: 'Alip', message: "[!] masukan parameter pp"
    })
    if (!bg) return res.json({
        status: false, creator: 'Alip', message: "[!] masukan parameter bg"
    })

    if (!imgpp) return res.json({
        status: false, creator: 'Alip', message: "[!] cek kembali url image pp"
    })
    if (!bgimg) return res.json({
        status: false, creator: 'Alip', message: "[!] cek kembali url image bg"
    })
...
router.get('/api/maker/goodbye1', cekKey, async (req, res, next) => {
    ...
    if (!imgpp) return res.json({
        status: false, creator: 'Alip', message: "[!] masukan parameter pp Link pp dengan betul"
    })
    if (!bgimg) return res.json({
        status: false, creator: 'Alip', message: "[!] masukan parameter pp Link bg dengan betul"
    })
...
router.get('/api/linkshort/tinyurl', cekKey, async (req, res, next) => {
    ...
    if (!link) return res.json({
        status: false, creator: `${creator}`, message: "[!] masukan parameter link"
    })

    var islink = isUrl(link)
    if (!islink) return res.json({
        status: false, creator: `${creator}`, message: "[!] masukan parameter url sahaja"
    })
...
router.get('/api/linkshort/tinyurlwithalias', cekKey, async (req, res, next) => {
    ...
    if (!alias) return res.json({
        status: false, creator: `${creator}`, message: "[!] masukan parameter alias"
    })
...
router.get('/api/info/githubstalk', cekKey, async (req, res, next) => {
    ...
    if (!user) return res.json({
        status: false, creator: `${creator}`, message: "[!] masukan parameter user"
    })
...
router.get('/api/info/translate', cekKey, async (req, res, next) => {
    ...
    if (!lang) return res.json({
        status: false, creator: `${creator}`, message: "[!] masukan parameter lang.  boleh lihat list bahasa di https://cloud.google.com/translate/docs/languages"
    })
    ...
    translate(text, {
        ...
        res.json({
            status: false, creator: `${creator}`, message: "[!] masukan parameter lang Dengan Betul.  boleh lihat list bahasa di https://cloud.google.com/translate/docs/languages"
        })
    })
...
router.get('/api/info/emoji', cekKey, async (req, res, next) => {
    ...
    if (!emoji1) return res.json({
        status: false, creator: `${creator}`, message: "[!] masukan parameter emoji"
    })
    var hasil = emoji.get(emoji1)
    if (hasil == null) return res.json({
        status: false, creator: `${creator}`, message: "[!] masukan parameter emoji dengan 1 emoji sahaja"
    })
    limitapikey(req.query.apikey)
    res.json({
        status: true,
...
router.get('/api/tools/ebase64', cekKey, async (req, res, next) => {
    ...
    if (text1.length > 2048) return res.json({
        status: false, creator: `${creator}`, message: "[!] Maximal 2.048 String!"
    })
    limitapikey(req.query.apikey)
